...
#### 1. 토큰 예치하기
amount만큼의 토큰을 예치하여, DefferedToken을 amount만큼 mint합니다.
...
#### 2. 토큰 인출 요청하기
amount 만큼의 토큰에 대해 인출 요청합니다.  Cooldown 기간 이후로 withdraw 호출하면 받을 수 있습니다.
````solidity
...
requestId를 통해 인출 요청 정보를 조회합니다.

````solidity
function withdrawalRequests(uint256 requestId) external view returns (WithdrawalRequest memory);

struct WithdrawalRequest {
    uint256 id;        // @dev 인출 요청 식별자, withdraw 시 parameter 로 활용
    uint256 amount;    // @dev 인출 요청 갯수
    uint256 requestTs; // @dev 인출 요청 시각, cooldown 시간 이후 호출 가능
    bool isClaimed;    // @dev 인출 여부 (조회 시 항상 false)
}
````

#### 4. 유저의 인출 요청 목록 조회하기
유저의 인출 요청에 대한 정보를 조회합니다. 만기 요청일 & 상환 받을 수 있는 갯수 등을 내려줍니다. 인출 시 목록에서 제거됩니다.
...
#### 5. 토큰 인출하기
인출 요청 수행하기. msg.sender로 토큰을 반환합니다  
